      ...
      <section className="py-24 px-6 overflow-hidden relative" style={{ background: 'linear-gradient(135deg, #eff6ff 0%, #f5f3ff 35%, #fdf2f8 65%, #fff7ed 100%)' }}>
        ...
        <div className="relative z-10 max-w-7xl mx-auto">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-10">
            {[
              { icon: Brain, stat: '3x', statLabel: 'Traffic', title: 'AI-Assisted Design', desc: 'AI helps us build websites that look stunning and are optimized for user engagement from day one.', color: 'text-blue-500', bg: 'bg-blue-100', border: 'border-blue-100' },
              { icon: Settings, stat: '95%', statLabel: 'SEO Score', title: 'Auto Performance', desc: 'Automated performance optimization ensures your site loads fast and ranks high on search engines.', color: 'text-violet-500', bg: 'bg-violet-100', border: 'border-violet-100' },
              { icon: BarChart3, stat: '40%', statLabel: 'More Leads', title: 'Behavior Insights', desc: 'User behavior tracking and heatmaps reveal what drives conversions and what holds them back.', color: 'text-emerald-500', bg: 'bg-emerald-100', border: 'border-emerald-100' },
              { icon: TrendingUp, stat: '∞', statLabel: 'Growth', title: 'Post-Launch Growth', desc: 'We continuously improve your website after launch — A/B testing, updates, and new features.', color: 'text-orange-500', bg: 'bg-orange-100', border: 'border-orange-100' },
            ].map((item, i) => (
          ...
          <motion.div initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }} transition={{ delay: 0.4 }} className="flex flex-wrap justify-center gap-3">
            {['Faster development timelines', 'SEO-friendly structure from day one', 'Higher conversion rates'].map((item) => (
