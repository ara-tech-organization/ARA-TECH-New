          ...
          <div className="grid lg:grid-cols-2 gap-16 items-center">
            <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6 }}>
              ...
              <p className="text-gray-500 text-lg leading-relaxed mb-8">
                Unlike traditional development companies, we focus on the full journey — from validating your idea to building and scaling a product that's market-ready from day one.
              </p>
              <div className="space-y-3">
                {[
                  { label: 'Idea validation before development', bg: 'bg-blue-50', icon: 'text-blue-500' },
                  { label: 'AI-powered product features built in', bg: 'bg-emerald-50', icon: 'text-emerald-500' },
                  { label: 'Scalability and automation from day one', bg: 'bg-violet-50', icon: 'text-violet-500' },
                ].map((item) => (
            ...
            <motion.div initial={{ opacity: 0, x: 20 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} transition={{ duration: 0.6, delay: 0.15 }}>
              <div className="rounded-3xl bg-violet-50 border border-violet-100 p-8 overflow-hidden relative">
                ...
                <div className="relative z-10 space-y-4">
                  {[
                    { icon: Lightbulb, label: 'Idea validation before development', color: 'bg-blue-100 text-blue-500' },
                    { icon: Brain, label: 'AI-powered product features', color: 'bg-violet-100 text-violet-500' },
                    { icon: Settings, label: 'Automation-driven architecture', color: 'bg-emerald-100 text-emerald-500' },
                    { icon: TrendingUp, label: 'Scalability from day one', color: 'bg-orange-100 text-orange-400' },
                  ].map((item, i) => (
